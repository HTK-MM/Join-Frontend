...
async function initLogin() {
  ...
  moveIcon();
  usersArray();
  /*usersJson = await loadData('auth/profile/');  */
}
...
async function getFirstNameUser(name) {
  let nameParts = name.split(' ');
  return nameParts[0];
}

async function getLastNameUser(name) {
  let nameParts = name.split(' ');
  return name.replace(nameParts[0] + ' ', '');  
}
/**
...
/**
 * Asynchronously checks if an email exists in the 'auth/profile/' data source.
 * 
 * @param {string} email - The email address to check for existence.
 * @return {Promise<boolean>} A promise that resolves to true if the email exists, or false otherwise.
 */

async function emailExists(email) {
  let usersJson = await loadData('auth/profile/');
