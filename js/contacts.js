...
async function contactsArray() {
  let contactsJson = await loadData('contact/');
  for (let contact in contactsJson) {
    contact = contactsJson[contact];
...
async function newContact(event) {
  ...
  let response = await postData('contact/', newContact);
  if (response != "") {
...
async function editContact(event, i) {
  ...
  let response = await serverUpdate(contactEdit);
  if (response != "") {
    closeDialog();
...
async function deleteContact(i) {
  ...
  let response = await serverDelete(contactDelete);
  if (response != "") {
    contacts.splice(i, 1);
 ...
 */
async function serverUpdate(contactEdit) {
  let contactsJson = await loadData('contact/');
  for (let contactDB in contactsJson) {
    contactDB = contactsJson[contactDB];
    if (contactDB.id == contactEdit.id) {
      let response = await putData('contact/' + [contactDB.id] + '/', contactEdit);
      return response;
    }
  }
}
 ...
 * @return {Promise<void>} A promise that resolves when the contact is successfully deleted.
 */
async function serverDelete(contactDelete) {
  let contactsJson = await loadData('contact/');
  for (let contactDB in contactsJson) {
    contactDB = contactsJson[contactDB];
    if (contactDB.id == contactDelete.id) {
      let response = await deleteData('contact/' + [contactDB.id] + '/');
      return response;
    }
  }
}
