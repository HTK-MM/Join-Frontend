...
function renderListContact() {
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    if (
      i == 0 || contact['name'].slice(0, 1) != contacts[i - 1]['name'].slice(0, 1)
    ) {
      contentList.innerHTML += `<div class="a-z-contact-list ">${contact['name'].slice(0, 1).toUpperCase()}</div>`;
    }
    ...
    <form class="add-contact-form" onsubmit=${functionNew}>
      <div class="group-contact-input">
        ...
        <div class="form-button">
          <button class="button-guest inputfield-text-style" type="button" onclick="closeDialog()">Close</button>
