  ...
  return /*html*/ `
      <label for="checkbox${i}">
          <li class="contactlist" id="contactList${i}">        
              ...
              <div class="contact-name" >${user.user.first_name} ${user.user.last_name}</div> 
              <input class="user-checkbox" onclick="showUsersEmblem()" type="checkbox" id="checkbox${i}" data-userid="${user.user.id}">          
          </li>
      </label>
        `;
}
...
/**
 * Renders an emblem HTML element for a user.
...
function renderEmblemUsers(user) {
  return /*html*/ `
      <div class="emblem" style="background-color: ${user.color}" id="${user.user.id}">
      ${user.emblem}
    </div>  `;
}
