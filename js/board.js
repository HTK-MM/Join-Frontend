...
function updateTasksByStatus(status, elementId) {
  ...
  if (filteredTasks.length == 0) {
    ...
    for (let i = 0; i < filteredTasks.length; i++) {
      ...
      renderProgressBar(filteredTasks[i].id, tasks);
    }
}
...
function showSmallUsersEmblem(task) {
  let smallUsersEmblem = document.getElementById(`smallUsersEmblem${task.id}`);
...
async function updateBoard(status) {
  ...
  for (let task in tasksJSON) {
    ...
    if (task.id == currentDraggedElement) {
      await patchData(`task/${task.id}/`, {status: status});
    }
  }
}
...
async function deleteTask(cardId) {
  let tasksJSON = await loadData('task/');
  for (let item in tasksJSON) {
    task = tasksJSON[item];
    if (task.id == cardId) {
      await deleteData(`task/${task.id}/`);
    }
  }
}
...
function getSelectedUserIds() {
  let checkboxes = document.querySelectorAll('.contactlist input[type="checkbox"]:checked');
  let selectedUserIds = [];
...
async function updateSubtasks(cardId, isubtask, value) {
  ...
  for (let key in tasksJSON) {
    ...
    if (task.id == cardId) {         
      task.subtask[isubtask].checked = value     
     let response = await patchData(`task/${cardId}/`, {subtask: task.subtask});
     if (response == ""){
      closeBigCard();
     }
    }
