...
function editAddSubtask() {
  ...
  if (input == '') {
    document.getElementById('editSubtaskInput').placeholder = 'Bitte etwas eingeben!';
    return;
  }
 ...
 */
function setEditPriorityStyles(selectedElement,backgroundColor,textColor,svgColor) {
  if (selectedElement) {
...
function showPickedUsersEmblems(cardId) {
  ...
  if (task && task.users) {
    for (let userId of task.users) {
      if (userId == 1) continue;
      let user = users.find((u) => u.user.id == userId);
    ...
    hiddenUserIds.clear();
    if (extraCount > 0) {
      let hiddenUsers = task.users.slice(5);
...
function checkUserCheckboxesBasedOnEmblems() {
  let renderedEmblems = document.querySelectorAll('#editUsersEmblem .edit-emblem');
...
async function updateEditBoard(cardId, updatedTask) {
  let tasksJSON = await loadData('task/');
  for (let task in tasksJSON) {
    task = tasksJSON[task];
    if (task.id == cardId) {
     await putData(`task/${task.id}/`, updatedTask);    
    }
  }
}
...
function getEditSelectedUserIds() {
  let checkboxes = document.querySelectorAll('.edit-contactlist input[type="checkbox"]:checked');
  let selectedUserIds = [];
